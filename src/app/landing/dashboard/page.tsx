...
// Components
import WarningNewsBell from "@/app/components/WarningNewsBell";
// types
...
const AWS_WEATHER_ICON_DIR = "/icon_weather";

/* -------------------- Functions -------------------- */

// Function ทำให้ weatherType เป็น string 2 หลัก (ถ้าเป็น null/undefined ให้เป็น empty string)
function normalizeWeatherType(weatherType?: string | number | null) {
    if (weatherType === null || weatherType === undefined) return "";
    return String(weatherType).trim().padStart(2, "0");
}

// Function เลือก icon path จาก weatherType 03 และ 63 (ถ้าไม่มีให้คืนค่า null)
function getAwsIconSrc(
    weatherType?: string | number | null,
    isNight = false
) {
    const type = normalizeWeatherType(weatherType);

    if (type === "03") {
        return isNight
            ? `${AWS_WEATHER_ICON_DIR}/4.CloudyNight.svg`
            : `${AWS_WEATHER_ICON_DIR}/3.CloudyDay.svg`;
    }

    if (type === "63") {
        return `${AWS_WEATHER_ICON_DIR}/5.Rain.svg`;
    }
...
function DashboardPage() {
    ...
    const awsIconSrc = getAwsIconSrc(awsItem?.weatherType, isNightNow);
